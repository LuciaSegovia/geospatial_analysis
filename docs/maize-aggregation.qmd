...
The file `predmaize.df` contains the coordinates of Malawi (lat/lon), the predicted maize Se concentration both log-transformed (Zhat) and back transformed (Zhat_exp) and their covariance (uncertainty associated to each prediction) for Malawi.

The data was derived from the `maize.df` dataset using ordinary kriging and the scripts can be found in `01_maize-model.R` and are based on the study of Gashu and colleagues (2022).

Then, in order to calculate the mean/median value for each level of aggregation we will use the boundaries that were generated in the script () and described in @sec-XX.
...
Here we are proceading as descibred in @sec-generating-the-boundaries. With the only difference in the buffers generation, as we are using higher diversity of buffers as we are less contrainded by the data as when we were using observed values.
...
To generate the final aggregations we are:

1. Loading the boundaries for the administrative units and the buffers which can be adapted using the function `buffer_generator()`. More information on @sec-buffer-area.
...
2. Converting the maize dataset (Long/Lat) into a spatial object
...
3. Merging the data by location: Because the dataset of the predicted maize is very large, and we only need the maize aggregation for the clusters were data on plasma Se were colected, we used a shapefile with the cluster-level spatial data only for those EAs/clusters. 
...
For the clusters it is performed equally but using a loop, that ingest the buffer generated and output the dataset with the maize Se mean for each buffer.
...
For all the datasets (n= `r length(file)`), except for the district level, the linkage between the women and its corresponding maize Se (i.e., the hypothetical concetration of Se that is likely to be supplied by maize) are based on the cluster id. This is because the buffer were generated based on the cluster centroids. 

For ease of use, the datasets linkages are done using a loop, which after the datasets are ready to be used in the R-INLA model.
...
There is a loop that links the plasma and the maize into one dataset. Then they are ready to be model using R-INLA. There is one of the covariates that it is not included in the data combination loop, and it is added in the INLA loop which is the distance to main lakes. 
...
Then, we generate a list to store all the models that are tested. 
