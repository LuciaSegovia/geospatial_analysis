...
Now, we are performing some summary statistics and checking the data skewness to decide on the logtranformation.
...
Due to the markedly right-side skewness, we decided to log-transform the variable (Se_raw). We can see in the figure below, that the log-transformed concentration of Se in maize is very close to normality, hence we will proceed with the log-trasformed variable for the variogram estimation. Variograms are particularly sensitive to right-side outliers (e.g., exceptionally large values) (@WwebsterGeostatisticsEnvironmentalScientists2007). 



```{r}

# Checking the data distrubution after log-transformation
summaplot(log(data.df[,element]),plotname2)

```

Then, we can see in @fig-ss the log-transformed maize Se concentration coloured by their quartile from the smallest (in blue) to the highest (in red) to see if there are some spatial trend or pattern. We also checked for North to South and West to East trends (see code in the R scripts). 
...
All these steps are key for getting selecting the best fitting model ang getting the most accurate results. The method for evaluating and selecting the model was based on @gashuNutritionalQualityCereals2021 and best practices suggested by  and @websterGeostatisticsEnvironmentalScientists2007. 

### Obtaining the variogram estimates

Now, we are studying the spatial structure of the data by generating an empirical variogram. 

To do so, first we need to calculate some information from the data: the distance between all pairs (lag) and the difference in maize Se concentration between two points. 
...
With that information, we are going to decide on the maximum lag distance that will be included in the variogram, and the lag bins. For more information about the use of bin distance (d) and the importance of its selection, see "*Irregular sampling in one dimension*" (@websterGeostatisticsEnvironmentalScientists2007, page 68). 

**To-DO:**Ask ML about the choice of 100km and 10km bins and potential differences in the estimates. 
...
Then, we for generating the variance we tested three different estimatiors: isotropic Matheron (`Ma`), Cressie-Hawkins (`CH`) and Dowd (`Do`).  
...
First, we need to choose initial estimates for the parameters. These will impact the estimates, so also should they be chosen with care! (See Chapter 6 and page 101 of @websterGeostatisticsEnvironmentalScientists2007).
