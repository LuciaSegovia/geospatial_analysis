...
We calculated the Body Mass Index (BMI) as it would be added to the model as a proxy for total food intakes. We assumed that women with higher food intake may have higher plasma Se due to higher dietary Se intake. For instance, a normal BMI (18.5-24.9 kg/m^2), would likely mean that the food intake was providing enough food (i.e, energy) to maintain a normal body weight. In Malawi, that food intake is normally represented by maize consumption, as it is the main staple crop which supplies more than 60% of the total energy supply (REF-Joy et al, , REF). Hence, we added this variable to account for potential differences in the Plasma Se due to (lower) food intake. 
...
```{r label = "fig-2_6a", fig.cap = "Variation in c-reactive protein (CRP) (mg/L) of women in Malawi by region and malaria test resutls."}
...
```{r label = "fig-2_6v", fig.cap = "Variation in  (AGP) (g/L) of women in Malawi by region and malaria test resutls."}
...
DHSDATA <- dhs.df %>% select(survey_cluster1, household_id1, LINENUMBER, survey_strata, PSU,
...
#TODO:
Check seasonal pattern for maize intake, sub-grouped by: maize fraction, wealth and urbanity (focus on rural). 
