...
## Generating the spatial aggregations
...
After identify and checking, we filtered only the correct district for each cluster, and saved as a dataset containing each cluster (`survey_cluster1`) and their corresponding district (`ADM2_EN`). 
...
That dataset (`district`) was used to exclude those EAs that were not in the corresponding district for each cluster. Note that there are two "district" missing, one is "Zomba City" and the other is "Likoma". We checked and the entries corresponding to Zomba were all rural. 
...
## Maize aggregation using predicted maize dataset
...
In order to improve the reporducibility, readability and transparency of the final model, we generated a function to obtain the buffers: `buffer_generator()`.

The funtion takes as inputs the spatial dataset with the point location of the centroids and the ids of each centroid, in our case the cluster id, and the minimum and maximum buffer size and the increments in which should be generated. 
...
The shapefiles generated for each buffer are stored in the `r print(folder)`. Then the folder was manually moved into the boundaries folder. This was needed because the `st_write()` can't deal with long names. See [GitHub issue](https://github.com/r-spatial/sf/issues/1792).
...
Here we can see the smallest buffer at 10km, followed by the maximum buffer at 60km.
...
Some test were performed to check the influence of some of the decission/assumptions made during the data preparation and processing. 
...
**TODO:check the variability of maize Se values by clusters**
