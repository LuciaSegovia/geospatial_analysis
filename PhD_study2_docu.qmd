...
### Geo-referenced crop (maize) composition (Se in maize) in Malawi
...
### Geo-referenced biomarkers data (Plasma Se in WRA) in Malawi 
...
- folder name (gadm40_MWI_shp): The file for the EAs has some issues that lead some points in the lake. Hence, we found a different file with the Malawi boudaries. 

- GeoBoundaries: It also has 3 level of boudaries: 1-region, 2-district, 3-EA.

### Geo-referenced apparent food consumption in Malawi (Se intake)

4. Fourth Integrated Household Survey 2016-2017. https://doi.org/10.48529/g2p9-9r19. It can be accessed through the World Bank microdata website. 
...
One of good source of countries boundaries are GADM, we were using those boundaries the problem is that they include lake Malawi as part of the boundaries of the EA/district / state. HEcne when using the boundaries to locate the HHs and/or maize it seems that part of the data is falling inside the lake. If we try to remove the lake area it leads to the missing of some observations. 

We are going to use other EA boundaries found [here]() frm the World Bank and used previously by the NE Red Cross. 
...
We have checked the new boundaries and they seemed correct, only that the areas when compared with the previous map are different. We are going to check that the distric reproted in the DHS survey are the same as those allocated by binding the EA bounderies file. 
...
Because of these tests should be run for all the datasets with boundaries. We have created a script where we are performing all these steps bedore the modelling, as this can be considered cleaning and processing previous the modelling. 
