...
-`maize.df`: This file contains the sample location (lat/lon), maize grain Se concentration, soil pH and Mean Annual Temperature (MAT). See section XX for more information on the dataset.

-`plasma.df`: This file contains the displaced cluster location (lon/lat), plasme Se concentration, and other individual level information (wealth quintile, BMI, etc.). See section XX for more information on the dataset.
...
The first step is to identify the Enumeration Areas where each maize grain were sampled. We started with the EAs because there are the smallest administrative units, and they are nested within the other (bigger) administrative units (Districts and Regions). 
