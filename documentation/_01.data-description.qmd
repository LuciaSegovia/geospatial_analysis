...
We checked wealth by region and it seemed that there is a slight unbalance for the lowest Q in the middle region and towards the highest Q in the Northern region. 
...
We extracted multiple variables of interest (including the LOD), which served to: 
    a) check the NAs values (n=427) in the dataset (and previously reported in the now excluded dataset in Gashu et al., 2021), this being higher means that some below detection were included in the MWI_CropSoilData_NA.csv (also found in M.L GeoNut dataset).
    b) to add those values as possible solution for accounting for the high number of missing Se conc. values.

Cleaning steps:

1. No measured: Six Se values were coded as not measured ("NM") (ID == c("MWI0574", "MWI1007","MWI1138","MWI1168","MWI1171","MWI1686")) (see MWI_CropSoilData_Raw.csv), which were transformed into `NA`. 
2. Negative values: 21 value were negative and converted to zero. 
