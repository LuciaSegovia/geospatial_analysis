...
It is important that we think about the displacement of the household location and the potential impacts on the final results. For instance, when two houeholds are in the same EA, the maize Se value is the same for both households. 
...
Predicted-mean Se in maize at different level of aggregation. According to Gashu et al, 2019 the covariates that were selected (significant) for the maize Se map (kringing map) were mean atmospheric temperature (MAT) and pH. 
